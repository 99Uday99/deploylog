            ...
            <table className="min-w-full text-sm text-left">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                    <tr>
                        ...
                        {stageNames.map(name => (
                            <th key={name} className="px-6 py-3 text-center">{name}</th>
                        ))}
                    </tr>
                </thead>
                <tbody>
                            ...
                            {stageNames.map(stageName => {
                                ...
                                return (
                                    <td key={stageName} className="p-2">
                                        {stage ? (
                                            <div className={`p-4 rounded-lg border flex flex-col items-center justify-center ${getStatusColor(stage.status)}`}>
                                                <span className="font-bold text-gray-700">{getDuration(stage.startedAt, stage.endedAt)}</span>
                                                {stage.status === 'failed' && <span className="text-xs text-red-600 font-bold mt-1">Failed</span>}
                                            </div>
                                        ) : (
